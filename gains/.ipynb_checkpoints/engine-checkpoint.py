...
__all__ = ["get_best", "molecular_similarity", "suppress_rdkit_sanity",
           ...
           "GeneSet", "Benchmark", "convex_search", "mutate"]
...
def mutate(parent, geneSet, get_fitness, target):
    ...
    def add_custom_fragment(childGenes, GeneSet, oldGene):
        ...
        newGene = Chromosome(geneSet.GetEntryDescription(
            random.sample(range(geneSet.GetNumEntries()), 1)[0]), 0, 0)
        ...
        try:
            childGenes = Chromosome(Chem.MolToSmiles(childGenes), 0, 0)
            return childGenes
    ...
    def add_rdkit_fragment(childGenes, GeneSet, oldGene):
        ...
        try:
            newGene = Chromosome(Chem.MolToSmiles(geneSet.GetFuncGroup(
                random.sample(range(geneSet.GetNumFuncGroups()), 1)[0])), 0, 0)
        except BaseException:
            return 0
        oldGene = oldGene + newGene.Mol.GetNumAtoms()
        combined = Chem.EditableMol(Chem.CombineMols(newGene.Mol,
        ...
        try:
            childGenes = Chromosome(Chem.MolToSmiles(combined.GetMol()), 0, 0)
            return childGenes
        except BaseException:
            return 0
    ...
    def remove_rdkit_fragment(childGenes, GeneSet, oldGene):
        ...
        try:
            ...
            return 0
    childGenes = Chromosome(parent.Genes, 0, 0)
    ...
    try:
        ...
        return Chromosome(parent.Genes, 0, 0), mutation
    ...
    try:
        ...
        return Chromosome(genes, fitness, prediction), mutation
    except BaseException:
        return Chromosome(parent.Genes, 0, 0), mutation


def get_best(get_fitness, optimalFitness, geneSet, display,
             show_ion, target, parent_candidates, mutate=mutate, seed=None,
    ...
    child : Chromosome object
        ...
        for details
    mutation_attempts : count of function calls
    """
    mutation_attempts = 0
    ...
    if bestParent.Fitness >= optimalFitness:
        return bestParent, mutation_attempts
    while True:
        with suppress_rdkit_sanity():
            child, mutation = mutate(bestParent, geneSet, get_fitness, target)
        mutation_attempts += 1
        ...
        if optimalCutOff:
            if greaterThanCutOff:
                if child.Prediction >= optimalCutOff:
                    ...
                    if verbose in [0, 1, 2]:
                        show_ion(child.Genes, target, mutation_attempts, sim_score,
                                 molecular_relative)
                    return child, mutation_attempts
            else:
                if child.Prediction <= optimalCutOff:
                    sim_score, sim_index = molecular_similarity(child,
                                                        parent_candidates)
...
class Chromosome(Chem.rdchem.Mol):
    ...
    """
    def __init__(self, genes, fitness, prediction):
...
def _generate_parent(parent_candidates, get_fitness, target):
    ...
    genes = df[ohPickMe[0]]
    fitness, prediction = get_fitness(genes, target)
    return Chromosome(genes, fitness, prediction)
