...
def generate_solvent(target, model_ID, heavy_atom_limit=50,
                     ...
                     mutation_cap=1000, mutate=genetic.mutate,
                     optimalCutOff=None, greaterThanCutOff=True):
    """
    ...
    for i, name in enumerate(model_ID):
        ...
        if i > 0:
            ...
            models = model
            deslists = deslist
    cols = ["Salt ID", "Total Attempts", "Salt Smiles", "Cation Heavy Atoms",
    ...
    if exp_data is not None:
        anion_candidates = np.array([i.split('.')[1] for i in exp_data])
    attempts = 0
    for i in range(1, hits + 1):
        while True:
            ...
            best, mutation_attempts = _guess_password(target, anion_smiles, parent_candidates,
                                   models, deslists, mutate=mutate, seed=seed, hull=hull,
                                   ...
                                   greaterThanCutOff=greaterThanCutOff)
            attempts += mutation_attempts
            if exp_data is not None:
                exp_parent_candidates = np.array([i.split('.')[0] for i in exp_data])
            ...
            if cation_heavy_atoms < heavy_atom_limit and \
                ...
                new_entry = pd.DataFrame([[salt_ID, attempts, salt_smiles,
                                           cation_heavy_atoms,
                                           tan_sim_score,
                                           molecular_relative,
                                           anion_name, pre]],
...
def _guess_password(target, anion_smiles, parent_candidates, models, deslists,
                    mutate=genetic.mutate, seed=None, hull=None, simplex=None,
                    exp_data=None, verbose=0, hull_bounds=[0, 1],
    ...
    best, mutation_attempts = genetic.get_best(fnGetFitness, optimalFitness, geneSet,
                            fndisplay, fnShowIon, target,
                            parent_candidates, mutate=mutate, seed=seed,
                            hull=hull, simplex=simplex,
                            verbose=verbose, hull_bounds=hull_bounds,
                            inner_search=inner_search,
                            parent_cap=parent_cap, mutation_cap=mutation_cap,
                            optimalCutOff=optimalCutOff, greaterThanCutOff=greaterThanCutOff)
    return best, mutation_attempts
...
def _display(candidate, mutation, startTime, scr, pre, target):
    ...
    """
    print("{}\t{:.5f}\t{}\t{}\t{}".format(
        candidate.Genes[:30].ljust(30), candidate.Fitness, mutation.ljust(20), pre, target))
...
def _show_ion(genes, target, mutation_attempts, sim_score, molecular_relative,
    ...
    anion = Chem.MolFromSmiles(anion_smiles)
    fitness, prediction = _get_fitness(anion, genes, target,
                                         models, deslists)
    anion_name = salty.check_name(anion_smiles)
    if exp_data is not None:
        chrom = genetic.Chromosome(genes, fitness, prediction)
        exp_parent_candidates = np.array([i.split('.')[0] for i in exp_data])
        tan_sim_score, sim_index = \
    ...
    print("{}\t{}".format("Model Prediction: ", prediction))
